    ...
    public class NoiseRemovalFileManager
    {
        ...
        private void RemoveNoiseWithFAST()
        {
            ...
            foreach (var path in CorruptedImagesPaths)
            {
                ...
                AMF(bytes, noiseMaps.ElementAt(fileNumber), fileNumber + 1, "fast");
                WAF(bytes, noiseMaps.ElementAt(fileNumber), fileNumber + 1, "fast");
                VMF(bytes, noiseMaps.ElementAt(fileNumber), fileNumber + 1, "fast");
                fileNumber++;
            }
        }
        ...
        private void RemoveNoiseWithFAPG()
        {
            ...
            {
                var bytes = File.ReadAllBytes(path);
                AMF(bytes, noiseMaps.ElementAt(fileNumber), fileNumber + 1, "fapg");
                WAF(bytes, noiseMaps.ElementAt(fileNumber), fileNumber + 1, "fapg");
                VMF(bytes, noiseMaps.ElementAt(fileNumber), fileNumber + 1, "fapg");
                fileNumber++;
            }
        }
        ...
        private List<bool[,]> ReturnFASTNoiseMaps()
        {
            ...
            foreach (var path in CorruptedImagesPaths)
            {
                ...
                File.WriteAllBytes(FileUtils.OutputMainFolderPath + "fast\\noise_maps\\noise_map" + fileNumber++.ToString() + ".bmp", manager.ReturnBytesFrom2DPixelArray());
                result.Add(noiseMap);
            }
            return result;
        }
        ...
        private List<bool[,]> ReturnFAPGNoiseMaps()
        {
            ...
            foreach (var path in CorruptedImagesPaths)
            {
                ...
                File.WriteAllBytes(FileUtils.OutputMainFolderPath + "fapg\\noise_maps\\noise_map" + fileNumber++.ToString() + ".bmp", manager.ReturnBytesFrom2DPixelArray());
                result.Add(fapg.DetectNoise());
            }
            return result;
        }

        private void AMF(byte[] bytes, bool[,] detectedNoise, int fileNumber, string detectionAlgorithm)
        {
            var manager = new PixelArrayManager(bytes);
            ...
            File.WriteAllBytes(FileUtils.OutputMainFolderPath + detectionAlgorithm + "\\AMF\\result" + fileNumber.ToString() + ".bmp", manager.ReturnBytesFrom2DPixelArray());
        }

        private void VMF(byte[] bytes, bool[,] detectedNoise, int fileNumber, string detectionAlgorithm)
        {
            var manager = new PixelArrayManager(bytes);
            ...
            File.WriteAllBytes(FileUtils.OutputMainFolderPath + detectionAlgorithm + "\\VMF\\result" + fileNumber.ToString() + ".bmp", manager.ReturnBytesFrom2DPixelArray());
        }

        private void WAF(byte[] bytes, bool[,] detectedNoise, int fileNumber, string detectionAlgorithm)
        {
            var manager = new PixelArrayManager(bytes);
            ...
            File.WriteAllBytes(FileUtils.OutputMainFolderPath + detectionAlgorithm + "\\WAF\\result" + fileNumber.ToString() + ".bmp", manager.ReturnBytesFrom2DPixelArray());
        }
    }
}
